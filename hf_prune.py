...
def main(args):
    ...
    model = AutoModelForCausalLM.from_pretrained(
        args.base_model, device_map="mps", torch_dtype="auto"
    )
    if args.device != "cpu":
        model.half()
    ...
    if args.test_before_train:
        ...
        ppl = PPLMetric(
            model, tokenizer, ["wikitext2", "ptb"], args.max_seq_len, device=args.device
        )
        ...
        for i in range(args.iterative_steps):
            ...
            if pruner_type in ["taylor"]:
                example_prompts = get_examples(
                    "bookcorpus", tokenizer, args.num_examples, seq_len=64
                ).to(args.device)
